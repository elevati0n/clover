...
module.exports = async ({ graphql, actions, reporter }, options) => {
  ...
  if (!data && response.errors) {
    ...
    data.allArticle.articles.length !== 0 &&
    (!options.blogSettings || !options.blogSettings.path)
  ) {
    throw new Error(
      `No path for ArticleListing page in gatsby-config specified`
    );
  ...
  const articleListingPageSlug = options.blogSettings.path.replace(
    /\/\/+/g,
    "/"
