...
module.exports = (
  { node, getNode, actions, createNodeId, reporter },
  options
) => {
...
function createSlugForFolder({ node, getNode }, options) {
  ...
  const shouldNotUsePathPrefix =
    options.blogSettings &&
    options.blogSettings.usePathPrefixForArticles === false;
  // Only if the user opts out or the blogSettings are not correctly specified, we switch to no path prefix
  const pathPrefix = shouldNotUsePathPrefix
    ? ""
    : options.blogSettings
    ? options.blogSettings.path
    : "";
